;(function ($, window, document, undefined) {
	...
	var defaults = {
		...
		'bounceMarker'               : true,
		'catMarkers'                 : null,
		'dataLocation'               : '/js/storeLocator/distributors_location_data/locations.json',
		'dataRaw'                    : null,
		'dataType'                   : 'json',
		'debug'                      : false,
		'defaultLat'                 : null,
		...
		'autoGeocodeErrorAlert'      : 'Automatic location detection failed because it no longer works on insecure origins.',
		...
		'nextPage'                   : 'Next >',
		'prevPage'                   : '< Prev',
		'prevPageDisabledColor'		: '#CCCCCC',
		'prevPageDisabledPointer'	: 'none',
		'nextPageDisabledColor'		: '#CCCCCC',
		'nextPageDisabledPointer'	: 'none'
	};
	...
	$.extend(Plugin.prototype, {
		...
		_start: function () {
			...
			if (_this.settings.fullMapStartBlank !== false) {
				...
				var styles = [{stylers: [{ hue: "#cc8800" },{ saturation: -100 },{ lightness: -20 }]}];var styledMap = new google.maps.StyledMapType(styles,{name: "Styled Map"});_this.map.mapTypes.set("map_style", styledMap);  _this.map.setMapTypeId("map_style");
		...
		_paginationOutput: function(currentPage, totalPages) {
			...
			// Previous page
			if ( currentPage > 0 ) { //if current page > 0, show enabled pointer 
				output += '<li class="bh-sl-next-prev" data-page="' + prevPage + '">' + this.settings.prevPage + '</li>';				
			}
			else //else if current page = 0, show disabled pointer
			{
				output += '<li class="bh-sl-next-prev" data-page="' + prevPage + '" style="color: ' + this.settings.prevPageDisabledColor + '; pointer-events: ' + this.settings.prevPageDisabledPointer + ';">' + this.settings.prevPage + '</li>'; //Disable Prev button with CSS
			}
			...
			// Next page
			if ( nextPage < totalPages ) { //if next page < totalPages, show enabled pointer 
				output += '<li class="bh-sl-next-prev" data-page="' + nextPage + '">' + this.settings.nextPage + '</li>';
			}
			else //else if next page > totalPages, show disabled pointer
			{
				output += '<li class="bh-sl-next-prev" data-page="' + nextPage + '" style="color: ' + this.settings.nextPageDisabledColor + '; pointer-events: ' + this.settings.nextPageDisabledPointer + ';">' + this.settings.nextPage + '</li>'; //Disable Next button with CSS
			}
		...
		autoGeocodeQuery: function (position) {
			...
			r.geocode({'latLng': latlng}, function (data) {
				if (data !== null) {
					...
					$("#origin_address").html(mappingObj.origin); //show the visitor's location on the distributor-inventory page
		...
		emptyResult: function() {
			...
			var styles = [{stylers: [{ hue: "#cc8800" },{ saturation: -100 },{ lightness: -20 }]}];var styledMap = new google.maps.StyledMapType(styles,{name: "Styled Map"});this.map.mapTypes.set("map_style", styledMap);  this.map.setMapTypeId("map_style");
