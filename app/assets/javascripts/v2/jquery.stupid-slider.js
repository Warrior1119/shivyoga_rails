(function() {
    ...
    function StupidSlider(el, options) {
        ...
        function init() {
			...
			$(".stupid-control:nth-child(1)").html("1. Add Members");
			$(".stupid-control:nth-child(2)").html("2. Registration Summary");
			$(".stupid-control:nth-child(3)").html("3. Payment");
        }
        ...
        function makeNextAndPrev() {
			...
			//$("#prevNextCtrls").wrapInner(nextCtrl);
			//$("#prevNextCtrls").wrapInner(prevCtrl);
			el.after(prevCtrl, nextCtrl);
			$('.stupid-prev,.stupid-next').wrapAll('<div id="prevNextCtrls"></div>');  
        }
        ...
        function makeControls() {
			...
			el.before(controlWrapper);	//Modified by Jayesh on 20 Jan, 2019
        }
        ...
        function next() {
            if ((currentIndex + 1) < slides.length) {
				...
				if(currentIndex == 1) //on 2nd Screen for Payment, show "Preview" button
				{
					$(".stupid-next").html("Go To Step 3 (Payment)");
					$('#slider').animate({scrollTop : 0},500);
				}
				if(currentIndex == 2) //on 3rd Screen for Preview, show "Confirm" button
				{
					$(".stupid-next").html("Confirm & Pay");
					setTimeout(function(){$(".stupid-next").addClass("confirm-btn")},300);
					$('#slider').animate({scrollTop : 0},500);
				}
				if (currentIndex == (slides.length-1))
				{
					//$(".stupid-next").addClass("disabled");
				}
				else
				{
					$(".stupid-prev").removeClass("disabled");
				}
            }	
        ...
        function previous() {
            if ((currentIndex - 1) >= 0) {
				...
				//console.log("Slides Length Prev: "+slides.length);
				if(currentIndex == 1) //on 2nd Screen for Payment, show "Preview" button
				{
					$(".stupid-next").html("Go To Step 3 (Payment)");
					$(".stupid-next").removeClass("confirm-btn");
					$('#slider').animate({scrollTop : 0},500);
				}
				if(currentIndex == 0) //on 1st Screen for Add Members, show "Payment" button
				{
					$(".stupid-next").html("Go To Step 2 (Summary)");
					$(".stupid-next").removeClass("confirm-btn");
					$('#slider').animate({scrollTop : 0},500);
				}
				if (currentIndex == 0)
				{
					$(".stupid-prev").addClass("disabled");
				}
				else
				{
					//$(".stupid-next").removeClass("disabled");
				}
